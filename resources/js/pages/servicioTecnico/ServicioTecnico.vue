...
<template>
    ...
    <AppLayout>
        <div class="flex h-full flex-1 flex-col gap-4 p-4 px-5 lg:px-20" style="background-color: #F4F4F4;">
            <!-- Header -->
            <div class="flex items-center gap-5 mt-4 lg:mt-10">
                <h1 class="text-2xl lg:text-[32px] font-bold text-gray-800">Servicio técnico</h1>
            </div>

            <!-- Top Controls -->
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                <!-- Tabs -->
                <div class="flex items-center gap-3 lg:gap-5 ml-1">
                    <div class="flex flex-col items-center">
                        <button @click="handle_filtro('EN PROCESO')" class="text-base lg:text-lg cursor-pointer"
                    ...
                    <div class="flex flex-col items-center">
                        <button @click="handle_filtro('FINALIZADOS')" class="text-base lg:text-lg cursor-pointer"
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full lg:w-auto">
                        ...
                        <div class="relative">
                            <input type="text" placeholder="Buscar" @input="handleSearch"
                                class="px-10 py-2 border rounded-full focus:outline-none text-black placeholder-[#0D509C] w-full sm:w-[200px]"
                        ...
                        <div class="flex gap-2">
                            <button @click="open_filtros = !open_filtros"
                                class="flex items-center justify-center gap-2 px-4 lg:px-6 py-2 bg-blue-600 text-white rounded-full cursor-pointer text-sm lg:text-base"
                            ...
                            <button v-if="can.gestionar" @click="modal_servicio = !modal_servicio"
                                class="flex items-center gap-2 px-4 lg:px-6 py-2 bg-blue-600 text-white rounded-full cursor-pointer text-sm lg:text-base whitespace-nowrap"
                                style="background-color: #0D509C;">
                                <span class="sm:hidden">+</span>
                                <span class="hidden sm:inline">Añadir Servicio</span>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <transition enter-active-class="transition-all duration-300 ease-in-out"
                ...
                <div v-if="open_filtros" class="text-black flex flex-col rounded-lg bg-white p-5">
                    <h2 class="font-semibold text-lg lg:text-xl mb-5">Filtrar por</h2>
                    <div class="flex flex-col lg:flex-row items-start lg:items-center gap-4 lg:gap-6">
                        <!-- Dates -->
                        <div class="flex-1 flex flex-col sm:flex-row items-start sm:items-center gap-4 lg:gap-6 w-full lg:w-auto">
                            <div class="flex flex-col w-full">
                                <label for="fecha_desde" class="text-sm lg:text-base" style="color: #5B5B5B;">Desde</label>
                                <input type="date" id="fecha_desde" v-model="fecha_desde" @change="handle_filtro_fechas"
                                    class="mt-1 p-2 w-full border border-gray-300 rounded-md">
                            </div>
                            <div class="flex flex-col w-full">
                                <label for="fecha_hasta" class="text-sm lg:text-base" style="color: #5B5B5B;">Hasta</label>
                        ...
                        <!-- Tecnico -->
                        <div v-if="can.gestionar" class="flex-1 flex flex-col w-full lg:w-auto">
                            <label for="tecnico_filter" class="text-sm lg:text-base" style="color: #5B5B5B;">Técnico</label>
                        ...
                        <div class="flex-1 flex flex-col w-full lg:w-auto">
                            <label for="direcciones" class="text-sm lg:text-base" style="color: #5B5B5B;">Dirección</label>
                            <select v-model="direccion_filter" @change="handle_filtro(null, direccion_filter)"
                                class="mt-1 p-2 w-full border border-gray-300 rounded-md">
                    ...
                    <div class="w-full flex justify-end mt-5">
                        <button @click="limpiar_filtros"
                            class="w-full sm:w-[180px] flex justify-center items-center py-2 bg-blue-600 text-white rounded-full cursor-pointer"
                            style="background-color: #0D509C;">
                            Limpiar filtros
                        </button>
                    </div>
                ...
                <table class="w-full bg-white overflow-hidden">
                    <thead class="bg-[#E1E5E9]">
                        <tr>
                            ...
                            <th class="py-3 px-4 text-center text-sm font-medium text-gray-600 uppercase tracking-wider">FECHA</th>
                            <th class="py-3 px-4 text-center text-sm font-medium text-gray-600 uppercase tracking-wider">N°</th>
                            <th class="py-3 px-4 text-center text-sm font-medium text-gray-600 uppercase tracking-wider">TÉCNICO</th>
                            <th class="py-3 px-4 text-center text-sm font-medium text-gray-600 uppercase tracking-wider">SERIE</th>
                            <th class="py-3 px-4 text-center text-sm font-medium text-gray-600 uppercase tracking-wider">FACTURA</th>
                            <th class="py-3 px-4 text-center text-sm font-medium text-gray-600 uppercase tracking-wider">MODELO</th>
                            <th class="py-3 px-4 text-center text-sm font-medium text-gray-600 uppercase tracking-wider">SALIDA</th>
                            <th class="py-3 px-4 text-center text-sm font-medium text-gray-600 uppercase tracking-wider">PROBLEMA</th>
                            <th class="py-3 px-4 text-center text-sm font-medium text-gray-600 uppercase tracking-wider">DIRECCIÓN</th>
                            <th class="py-3 px-4 text-center text-sm font-medium text-gray-600 uppercase tracking-wider">ESTADO</th>
                            <th class="py-3 px-4 text-center text-sm font-medium text-gray-600 uppercase tracking-wider">PAGADO</th>
                            <th class="text-center text-sm font-medium text-gray-600 uppercase tracking-wider"></th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        <tr v-for="(servicio, index) in servicios" :key="index">
                            ...
                            <td class="py-3 px-4 text-sm text-center text-gray-800">{{ servicio.user?.name }} {{ servicio.user?.apellido }}</td>
                            ...
                            <td class="py-3 px-4 text-sm text-center text-gray-800">{{ servicio.modelo.nombre_modelo }}</td>
                            ...
                            <td class="text-center">
                                <button @click="handle_change_pagado(servicio, !servicio.pagado)"
                                    class="cursor-pointer flex justify-center items-center w-full h-full">
                                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path v-if="!servicio.pagado"
            ...
            <div class="md:hidden mt-4 space-y-4">
                <div v-for="(servicio, index) in servicios" :key="index"
                    ...
                    <!-- Header with status indicator and number -->
                    <div class="flex justify-between items-center mb-3 pb-3 border-b border-gray-100">
                        <div class="flex items-center gap-2">
                            ...
                            <span v-else class="inline-block w-4 h-4 rounded-full bg-green-600"
                                title="No reincidente"></span>
                            <span class="font-semibold text-gray-800">N° {{ servicio.id }}</span>
                        </div>
                        <span class="text-sm text-gray-500">{{ servicio.created_at }}</span>
                    </div>
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between">
                            ...
                            <span class="text-sm text-gray-800 text-right">{{ servicio.user?.name }} {{ servicio.user?.apellido }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-sm font-medium text-gray-600">Serie:</span>
                            <span class="text-sm text-gray-800">{{ servicio.serie }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-sm font-medium text-gray-600">Factura:</span>
                            <span class="text-sm text-gray-800">{{ servicio.factura }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-sm font-medium text-gray-600">Modelo:</span>
                            <span class="text-sm text-gray-800 text-right">{{ servicio.modelo.nombre_modelo }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-sm font-medium text-gray-600">Salida:</span>
                            <span class="text-sm text-gray-800">{{ servicio.fecha_salida }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-sm font-medium text-gray-600">Problema:</span>
                            <span class="text-sm text-gray-800 text-right">{{ servicio.problema.nombre }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-sm font-medium text-gray-600">Dirección:</span>
                            <span class="text-sm text-gray-800 text-right">{{ servicio.direccion }}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between items-center mt-4 pt-3 border-t border-gray-100">
                        <div class="flex items-center gap-2">
                            ...
                            <div class="border rounded-2xl flex items-center justify-center py-1 px-2">
                                <span class="inline-block w-3 h-3 rounded-full mr-1" :class="{
                                    'bg-yellow-300': servicio.estado === 'Pendiente',
                                    'bg-green-500': servicio.estado === 'Finalizado',
                                    'bg-red-500': servicio.estado === 'Urgente',
                                }"></span>
                        ...
                        <div class="flex items-center gap-3">
                            <!-- Payment status -->
                            <button @click="handle_change_pagado(servicio, !servicio.pagado)"
                                class="cursor-pointer" :title="servicio.pagado ? 'Pagado' : 'No pagado'">
                                <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path v-if="!servicio.pagado"
                                        d="M16 0H2C1.46957 0 0.960859 0.210714 0.585786 0.585786C0.210714 0.960859 0 1.46957 0 2V16C0 16.5304 0.210714 17.0391 0.585786 17.4142C0.960859 17.7893 1.46957 18 2 18H16C16.5304 18 17.0391 17.7893 17.4142 17.4142C17.7893 17.0391 18 16.5304 18 16V2C18 1.46957 17.7893 0.960859 17.4142 0.585786C17.0391 0.210714 16.5304 0 16 0ZM16 2V16H2V2H16Z"
                                        fill="#D9D9D9" />
                            ...
                            <!-- Details arrow -->
                            <button @click="servicio_tecnico_detalle(servicio.id)"
                                class="hover:bg-neutral-200 p-2 rounded-full duration-300 cursor-pointer">
                                <svg width="11" height="18" viewBox="0 0 11 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M1 1L9 9L1 17" stroke="#0C0C0C" stroke-width="2" />
                                </svg>
                            </button>
        ...
        <!-- Modal servicio -->
        <div v-if="modal_servicio" @click.self="modal_servicio = false"
            class="fixed inset-0 bg-opacity-50 flex justify-center items-center z-50 p-4"
            style="background-color: rgba(0, 0, 0, 0.5);">
            <div class="bg-white rounded-lg p-4 lg:p-6 w-full max-w-[645px] overflow-y-auto max-h-[90vh] transform transition-all duration-300 scale-100">
                <h2 class="text-lg lg:text-xl font-semibold mb-4 text-black">Nuevo Servicio técnico</h2>

                <div class="w-full space-y-4">
                    <!-- Serie y Modelo -->
                    <div class="w-full flex flex-col lg:flex-row gap-4 lg:gap-5">
                        ...
                        <div class="w-full lg:w-1/2">
                            <div class="flex items-center gap-2">
                                ...
                                <div v-if="loadingModelo"
                                    ...
                                    <svg class="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 200 200">
                                        <radialGradient id="a13" cx=".66" fx=".66" cy=".3125" fy=".3125"
                                            gradientTransform="scale(1.5)">
                                            <stop offset="0" stop-color="#0D509C"></stop>
                                            <stop offset=".3" stop-color="#0D509C" stop-opacity=".9"></stop>
                                            <stop offset=".6" stop-color="#0D509C" stop-opacity=".6"></stop>
                                        ...
                                        <circle transform-origin="center" fill="none" stroke="url(#a13)"
                                            stroke-width="15" stroke-linecap="round" stroke-dasharray="200 1000"
                                            stroke-dashoffset="0" cx="100" cy="100" r="70">
                                            <animateTransform type="rotate" attributeName="transform" calcMode="spline"
                                                dur="2" values="360;0" keyTimes="0;1" keySplines="0 0 1 1"
                    ...
                    <!-- DNI/CUIT y Cliente/Distribuidor -->
                    <div class="w-full flex flex-col lg:flex-row gap-4 lg:gap-5">
                        <div class="w-full lg:w-1/2">
                    ...
                    <div class="w-full flex flex-col lg:flex-row gap-4 lg:gap-5">
                        ...
                        <div class="w-full lg:w-1/2">
                            ...
                            <select v-model="form.provincia" class="mt-1 p-2 w-full border border-gray-300 rounded-md">
                                <option value="">Seleccionar provincia</option>
                                <option v-for="provincia in provincias" :key="provincia.nombre" :value="provincia.nombre">
                                    {{ provincia.nombre }}
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="w-full lg:w-1/2">
                            <div class="flex items-center gap-2">
                                ...
                                <div v-if="loading_subproblemas"
                                    ...
                                    <svg class="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 200 200">
                                        <radialGradient id="a14" cx=".66" fx=".66" cy=".3125" fy=".3125"
                                            gradientTransform="scale(1.5)">
                                            <stop offset="0" stop-color="#0D509C"></stop>
                                            <stop offset=".3" stop-color="#0D509C" stop-opacity=".9"></stop>
                                            <stop offset=".6" stop-color="#0D509C" stop-opacity=".6"></stop>
                                        ...
                                        <circle transform-origin="center" fill="none" stroke="url(#a14)"
                                            stroke-width="15" stroke-linecap="round" stroke-dasharray="200 1000"
                                            stroke-dashoffset="0" cx="100" cy="100" r="70">
                                            <animateTransform type="rotate" attributeName="transform" calcMode="spline"
                                                dur="2" values="360;0" keyTimes="0;1" keySplines="0 0 1 1"
                            ...
                            <select v-model="form.subproblema_id" name="subproblema"
                                id="subproblema_id" class="mt-1 p-2 w-full border border-gray-300 rounded-md">
                                <option value="">Seleccionar sub problema</option>
                    ...
                    <div class="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 pt-4">
                        <button @click="modal_servicio = false"
                            class="w-full sm:w-auto px-6 py-2 bg-white text-[#0D509C] border border-[#0D509C] hover:shadow-md rounded-full duration-300 cursor-pointer">
                            Cancelar
                        </button>
                        <button @click="create_servicio_tecnico"
                            class="w-full sm:w-auto px-6 py-2 bg-[#0D509C] text-white rounded-full hover:shadow-md duration-300 cursor-pointer">
                            Guardar
                        </button>
                    </div>
                </div>
