...
const imprimirEtiqueta = () => {
    ...
    const url = `/operarios/etiqueta/${modalPrint.operario.id}`;
                        ...
                        <tr v-for="(operario, index) in props.operarios" :key="index">
                            ...
                            <td class="py-3 px-4 text-center flex items-center justify-center gap-4">
                                ...
                                <button class="cursor-pointer hover:opacity-70" @click="abrirModalImprimir(operario)">
                                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M15 4H13V0H5V4H3C1.34 4 0 5.34 0 7V12H3V18H15V12H18V7C18 5.34 16.66 4 15 4ZM7 2H11V4H7V2ZM13 16H5V11H13V16ZM15 9C14.45 9 14 8.55 14 8C14 7.45 14.45 7 15 7C15.55 7 16 7.45 16 8C16 8.55 15.55 9 15 9Z" fill="#0D509C"/>
                                    </svg>
                                </button>
                                <!-- Botón Editar -->
                ...
                <div class="grid grid-cols-2 gap-3">
                    <label v-for="sector in props.sectores" :key="sector.id" class="flex items-center gap-2">
                        <input 
                            ...
                            @change="toggleSector(sector.id, modalCreate.data.sectores)"
                            class="w-4 h-4" 
                        />
                ...
                <div class="grid grid-cols-2 gap-3">
                    <label v-for="sector in props.sectores" :key="sector.id" class="flex items-center gap-2">
                        <input 
                            type="checkbox" 
                            :value="sector.id"
                            :checked="modalEdit.data.sectores.includes(sector.id)"
                            @change="toggleSector(sector.id, modalEdit.data.sectores)"
                            class="w-4 h-4" 
                        />
                        <span class="text-sm text-gray-700">{{ sector.nombre }}</span>
                    </label>
        ...
        <div class="w-full max-w-[500px] bg-white rounded-lg p-6">
            ...
            <div class="text-center mb-6">
                <div class="bg-gray-50 p-6 rounded-lg mb-4">
                    <h3 class="text-lg font-semibold mb-2">{{ modalPrint.operario?.nombre }} {{ modalPrint.operario?.apellido }}</h3>
                    <p class="text-gray-600 mb-4">Legajo: {{ modalPrint.operario?.n_legajo }}</p>
                    
                    <!-- Código de barras SVG real -->
                    <div class="flex justify-center mb-2">
                        <div class="bg-white p-4 border rounded">
                            <img v-if="modalPrint.operario" 
                                 :src="`/operarios/barcode/${modalPrint.operario.id}`" 
                                 :alt="'Código de barras: ' + modalPrint.operario.codigo_qr"
                                 class="mx-auto max-w-full h-auto mb-2"
