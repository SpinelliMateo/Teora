...
class PrearmadoController extends Controller
{
    ...
    public function imprimirZebra(Request $request)
    {
        $controlStock = ControlStock::find($request->controlStock_id);
        
        if (!$controlStock) {
            return response()->json(['success' => false, 'message' => 'Control de stock no encontrado']);
        }
        ...
        if ($resultado) {
            return response()->json(['success' => true, 'message' => 'Etiqueta impresa correctamente']);
        } else {
            return response()->json(['success' => false, 'message' => 'Error al enviar a la impresora']);
        }
    }
